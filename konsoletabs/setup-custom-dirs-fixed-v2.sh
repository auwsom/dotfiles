...
# Based on the working setup with proper sudo command and directory paths
...
# First set up the initial tab using working sudo command pattern
...
# Create 2 additional tabs - each gets sudo command AFTER creation
for ((i=1; i<3; i++)); do
    echo "Creating tab $极in Window 4"
    xdotool key ctrl+shift+t
    sleep 1  # Wait for tab to be created
    # NOW run the sudo command in the new tab
    xdotool type "sudo -u aimgr bash -c 'cd /home/aimgr/dev/avoli/agent3 && exec bash'"
...
echo "Using working sudo -u aimgr bash -c pattern ✓"
echo ""
