#!/bin/bash
# vm_backup_final.sh - VM backup with timeout and error handling
...
echo "Attempting blockcopy with 30s timeout..."
if sudo timeout 30 virsh blockcopy "$VM_NAME" vda "$BACKUP_FILE" --transient-job 2>&1; then
