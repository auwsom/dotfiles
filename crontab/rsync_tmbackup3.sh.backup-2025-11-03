...
PRUNE_THEN_BACKUP=false              # Added - prune before backup for large files
...
fn_display_usage() {
    ...
    echo " --prune-then-backup       Prune old backups before starting the backup (useful for large files)."
...
# -----------------------------------------------------------------------------
# Handle --prune-then-backup option (New Block)
# -----------------------------------------------------------------------------
if [ "$PRUNE_THEN_BACKUP" = true ]; then
    fn_log_info "Pruning old backups before starting backup (strategy: $EXPIRATION_STRATEGY)"
    # Apply strategy to all existing backups to free up space
    fn_expire_backups ""
    fn_log_info "Pre-backup pruning completed. Starting backup..."
fi
