...
# 🚀 CONTAINER TEST RUN SCRIPT - WITH OUTPUT LOGGING
# Runs chat.py --test with core isolation, OOM killer disabled, and output logging

set -e  # Exit on any error

echo "🚀 Starting Container Mesh Test (With Output Logging)..."
echo "================================"
...
# Execute container with resource isolation (OOM killer disabled) and output logging
docker run --rm \
  --name "$CONTAINER_NAME" \
  --user 1003 \
  --memory="$MEMORY_LIMIT" \
